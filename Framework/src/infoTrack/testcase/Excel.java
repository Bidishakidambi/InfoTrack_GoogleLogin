...
public class Excel {
	...
	public Outcome getScript(String testCases) {
		...
		try {
			...
			while(token.hasMoreTokens()){
				...
				if( sheet == null ){
					System.out.println( testCase + ": Test Case Not found " );
					continue;
				}
	...
	public Outcome getconfiguration() {
		...
		try {
			...
			TestScript configuration = new TestScript();
			DataFormatter df = new DataFormatter();
			...
			for(int r = 1; r < totalRow; r++) {
				...
				configuration.addRow(rowData);
			}

			Outcome outcome = new Outcome();
			outcome.setStatus(true);
			outcome.setScript(configuration);
			return outcome;
		} catch (FileNotFoundException e) {
			System.out.println("File not found. ( " + this.filePath + ")");
			return new Outcome();
		} catch (IOException e) {
			System.out.println("Error in reading (" + this.filePath + ")");
			return new Outcome();
		}
	}
}
